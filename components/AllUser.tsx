...
import { FileEdit, Plus, Search, Trash2, User } from "lucide-react"
...
const columns = [
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center font-semibold capitalize cursor-pointer text-primary">
        <span className="flex items-center justify-center w-5 h-5 mr-1 border-2 border-orange-500 rounded-full">
          <User className="text-orange-500 size-5" />
        </span>
        <Link href={`/sales/${row.getValue("name")}`}>
          {row.getValue("name")}
        </Link>
      </div>
    ),
...
export default function ManageUser() {
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between w-full px-2 mt-5 bg-orange-200 rounded-md ">
